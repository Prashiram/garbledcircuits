...
int main(int argc, char** argv)
{
	...
	recv(alice, &n, 4, 0);
	n = ntohl(n);
	recv(alice, buffer, n, 0);    //A8
	fprintf(random,"%s\n",buffer);
	bzero(buffer, strlen(buffer));

	recv(alice, &n, 4, 0);
	n = ntohl(n);
	recv(alice, buffer, n, 0);    //A9
	fprintf(random,"%s",buffer);
	bzero(buffer, strlen(buffer));

	fclose(random);
	printf("Randomly generated strings received from Alice\n");
